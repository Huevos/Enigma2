...
SystemInfo["Scart-YPbPr"] = getBrandOEM() == "vuplus" and "4k" not in getBoxType()  # Machines that do have SCART component video (red, green and blue RCA sockets).
...
SystemInfo["no_YPbPr"] = getHaveYUV() in ("False",)  # Machines that do not have component video (red, green and blue RCA sockets).
SystemInfo["no_yellow_RCA__no_scart"] = getHaveRCA() in ("False",) and (getHaveSCART() in ("False",) and getHaveAVJACK() in ("False",))  # Machines that have neither yellow RCA nor Scart sockets.
SystemInfo["supportPcmMultichannel"] = fileCheck("/proc/stb/audio/multichannel_pcm")
SystemInfo["yellow_RCA_no_scart"] = getHaveSCART() in ("False",) and (getHaveRCA() in ("True",) or getHaveAVJACK() in ("True",))  # Machines that have composite video (yellow RCA socket) but do not have Scart.

# These entries are out of sequence as they depend on earlier SystemInfo values...
#
